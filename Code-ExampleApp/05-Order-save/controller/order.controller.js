sap.ui.define([
   ...
   const orderController = oController.extend("oum.controller.order", {
      ...
      handleProductSelected: function(oEvent) {
         ...
         this.getEditedOrder().addOrderLine(productData.isbn, 1);
         this.handleOrderChanged();
      },
      ...
      saveRecord: function() {
         const orderData = this.getEditedOrder().getData();
         if (orderData.status === "new") {
            orderData.orderDate = new Date();
         }
         
         const orderSpecs = oui5lib.mapping.getEntityAttributeSpecs("order");
         const errors = oui5lib.validation.validateData(orderData, orderSpecs);
         if (errors.length > 0) {
            oui5lib.ui.showValidationErrors(errors);
            return;
         }

         delete orderData.billingName;
         delete orderData.shippingName;
         delete orderData.total;
         delete orderData.statusText;
         delete orderData.valueState;
         const orderLines = orderData.items;
         orderLines.forEach(function(orderLine) {
            delete orderLine.productName;
            delete orderLine.lineTotal;
         });
      ...
      saveOrder: function(orderData) {
         ...
         oui5lib.request.sendMappingRequest(
            "order", "saveOrder",
            { "orderString": JSON.stringify(orderData) },
            this.orderSaved.bind(this)
         );
      },
      orderSaved: function(responseObject) {
         ui.setBusy(false);

         if (!responseObject.result) {
            oui5lib.messages.showErrorMessage(
               oui5lib.util.getI18nText("order.saved.error"));
            return;
         }

         const orderData = responseObject.value;
         orders.addData(orderData);

         oum.message = {
            text: oui5lib.util.getI18nText("order.saved.success"),
            type: "Success"
         };
         this.getRouter().vNavTo("ordersList");
         this.resetView();
      },
      ...
      resetView: function() {
         ...
         const orderForm = this.getView().byId("orderForm");
         orderForm.setModel(null, "order");

         const orderLinesTable = this.getView().byId("orderLinesTable");
         orderLinesTable.setModel(null, "items");

         this.getView().setModel(null, "state");
      },
      revertChanges: function() {
         this.resetRecordChanged();

         const originalOrder = new Order(this.orderId, true);
         this.setEditedOrder(originalOrder);
         this.setOrderModels();
      },
